...
- **`table_truncated` — a new verifier flag for tables that survive but lose most of their rows.** GT-0 measured this blind spot: `_has_table` tests *presence*, so a table cut from 40 rows to 3 still has ≥2 pipe rows and passes. The new check compares cardinality both document-wide and for the largest contiguous table block — the block test is what catches truncation of one table among several, where the document-wide total barely moves.
  - **Measured on the pre-registered GT-0 corpus (24 documents / 40 ground-truth files — 8 documents are double-annotated, so they appear three times):** of the **7 truncatable files, covering 5 distinct documents**, the old presence check missed **7 of 7** and the new flag catches **7 of 7**; **0 false positives across all 40 unmodified ground-truth files**. Those 5 documents are the same ones behind the previously published "5 seeded cases" — the file counts are higher only because double-annotated documents are counted once per pass, and are stated that way rather than as a larger sample.
  - **Not yet reproducible from `validate_verifier.py`** — that script does not compute `table_truncated`, and `VERIFIER-VALIDATION.md` still reports the pre-fix result. Folding the new check into the script and regenerating the report is outstanding; until then this figure is measured but not independently regenerable.
